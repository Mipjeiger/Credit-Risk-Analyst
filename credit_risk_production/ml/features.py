...
def _sd(a, b):
    try:
        return (
            float(a) / float(b)
            if pd.notna(a) and pd.notna(b) and b not in (0, None)
            else 0.0
        )
...
def add_derived_features(row: pd.Series) -> pd.Series:
    ...
    # Derived features
    r["delinq_velocity"] = _sd(
        row.get("num_deliq_6mts", 0), (row.get("num_deliq_12mts", 0) or 0) + 1
    )
    r["enq_velocity"] = _sd(row.get("enq_L3m", 0), (row.get("enq_L12m", 0) or 0) + 1)
    r["unsecured_ratio"] = _sd(
        row.get("Unsecured_TL", 0), (row.get("Total_TL", 0) or 0) + 1
    )
    r["active_ratio"] = _sd(
        row.get("Tot_Active_TL", 0), (row.get("Total_TL", 0) or 0) + 1
    )
    r["recent_open_ratio"] = _sd(
        row.get("Total_TL_opened_L6M", 0), (row.get("Total_TL", 0) or 0) + 1
    )
    r["dpd_score"] = float(row.get("num_times_30p_dpd", 0) or 0) + 2 * float(
        row.get("num_times_60p_dpd", 0) or 0
    )
    r["asset_class_score"] = (
        float(row.get("num_sub", 0) or 0)
