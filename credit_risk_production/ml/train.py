...
def run():
    ...
    for col in cat_cols:
        ...
        )  # This will raise an error if unseen categories exist
        le_map[col] = le
    ...
    for name, model in models.items():
        with mlflow.start_run(run_name=name):
            ...
            mlflow.log_metrics(
                {k: float(v) for k, v in metrics.items() if not np.isnan(v)}
            )
    ...
    print(
        f"✅ Trained {len(tuned_models)} models. Best: {best['name']} (F1={best['f1']:.4f}) ⚡"
    )
