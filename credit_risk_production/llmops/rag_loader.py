...
class CreditRiskRAG:
    def __init__(self, base_dir: Path = LLM_PATH):
        ...
        self.manifest = json.loads(
            (self.base / "outputs_llm" / "pipeline_manifest.json").read_text()
        )
        self.model_bundle = joblib.load(
            self.base / "outputs_llm" / "model_artifacts" / "model_bundle.joblib"
        )
    ...
    def decide(self, row: pd.Series) -> dict[str, Any]:
        ...
        # RAG path
        q = row.to_json()
        cust = self.customer_store.similarity_search(
            q, k=self.cfg["thresholds"]["k_customers"]
        )
        pol = self.policy_store.similarity_search(
            q, k=self.cfg["thresholds"]["k_policies"]
        )
        ctx_c = "\n\n".join(d.page_content[:600] for d in cust)
        ctx_p = "\n\n".join(
            f"[{d.metadata.get('doc_type')}] {d.page_content}" for d in pol
        )
        ...
        txt = re.sub(
            r"^```(json)?|```$", "", out["text"].strip(), flags=re.MULTILINE
        ).strip()
        try:
